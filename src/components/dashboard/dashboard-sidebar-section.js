...
const renderNavItems = ({ depth = 0, items, path, user }) => (
  <List disablePadding>
    {items.reduce(
      (acc, item) => reduceChildRoutes({ acc, depth, item, path, user }),
      []
    )}
  </List>
);

const reduceChildRoutes = ({ acc, depth, item, path, user }) => {
  ...
  if (user && item.role && item.role.includes(user.role)) {
    if (item.children) {
      acc.push(
